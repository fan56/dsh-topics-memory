...
// inject never resolves so the /topics command body is untouched here.
function mockCtx() {
  const registered = []
  const ctx = {
    registered,
...
test('plugin metadata: name and inject expose the skills dependency', () => {
  assert.equal(name, 'dsh-topics-memory')
...
test('provider.get loads the packaged SKILL.md with matching metadata', async () => {
  ...
  assert.ok(definition.content.includes('# dsh-topics-memory 使用指南'), 'body must be the packaged skill markdown')
