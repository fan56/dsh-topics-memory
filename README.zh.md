...
## 为什么做这个插件（作者的 memory 观）

**记忆不是越多越好。** 大多数「记忆」类产品的默认假设——能记就记、能查多查——对 LLM 恰恰是反的：模型注意力有限，巨型记忆库每一轮都在稀释注意力；而大量错误的过程记忆会把模型引向错误的决定。本插件押的是反面：

- **记忆重在质量，质量靠编辑。** 长时记忆的单位是 **topic**：一个 topic 记录的是这件事的**起始问题、结论、影响、可能的依赖关系**。**过程并不重要**——过程属于 session，session 结束就该让它过去；只有蒸馏出的结论才配进 bundle。
- **短时记忆，session 就够了。** 对话上下文本就负责它；把 session 回喂给模型的「记忆」是噪音。
- **长时记忆，topics 对 LLM 更友好。** 小而结构化、git 可追溯、按预算切片注入：注入走免 LLM 的词法匹配（毫秒级），per-topic ≤300 token、每轮 ≤1.5k token 封顶、零命中零注入——设计目标是**每轮给模型最小的高价值上下文**，不是最大召回。

所以这条管线本质是一条**编辑管线**：会话产出原子观察，后台蒸馏成 topic；最终被注入的只有 topic（问题、结论、影响、依赖）。模型看到的，是一位靠谱同事会当面 briefing 的内容——不是会议录音。
